...
// 'normal' prospects converge toward their true ovr/potential as scouting
// progresses; 'sleeper'/'bust' prospects have a scouting report permanently
// mis-centered away from the truth — scouting narrows how *confident* the
// report is, not how *correct* it is.
export type ProspectVariance = 'normal' | 'sleeper' | 'bust'
...
export interface DraftProspect {
  ...
  pos: string
  age: number
  // True, hidden ovr/potential — ground truth read by CPU draft logic and
  // revealed to the human only once `drafted` flips true. Everything below
  // is the human-visible scouting layer built on top of it.
  ovr: number
  potential: string
  origin: string
  pts: string
  ...
  scoutedOvrRange: ScoutedRange
  // [worst-case, best-case] letter grade, e.g. ['C+', 'A-'].
  scoutedPotentialRange: [string, string]
  scoutingProgress: number
  varianceType: ProspectVariance
  combineRevealed: boolean
...
export type AgentResponseKind = 'pending' | 'accepted' | 'countered' | 'rejected' | 'signedElsewhere'

export interface FreeAgentNegotiation {
  playerId: number
  myOffer: FreeAgentOffer | null
  rivalOffers: FreeAgentOffer[]
  agentResponse: AgentResponseKind
  agentMessage: string
  patience: number
  marketValue: number
  reSignBias: number
...
export type OffseasonSubPhase = 'combine' | 'draft' | 'moratorium' | 'freeAgency' | 'trainingCamp'

// Day-offsets (from the offseason's day-0 start, the day the Finals end) at
// which each offseason milestone begins — same "day counter" idiom as
// SeasonCalendar.seasonEndDay, just for the offseason's own calendar.
export interface OffseasonCalendar {
  startDate: SimDate
  combineDay: number
  draftDay: number
  faOpenDay: number
