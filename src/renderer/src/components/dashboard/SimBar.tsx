...
export default function SimBar(): React.JSX.Element {
  ...
  const simOffseasonDay1 = useGameStore((s) => s.simOffseasonDay1)
  const simOffseasonWeek1 = useGameStore((s) => s.simOffseasonWeek1)
  const simOffseasonMonth1 = useGameStore((s) => s.simOffseasonMonth1)
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
        ...
        >
          {seasonPhase === 'offseason'
            ? `S${seasonNumber} · OFFSEASON · ${offseasonPhase?.toUpperCase() ?? ''}`
            : `S${seasonNumber} · D${simDay}/${gamesPerSeason}`}
        </span>
