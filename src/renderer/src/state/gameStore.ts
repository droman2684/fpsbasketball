...
import {
  advanceToNextSeason as advanceToNextSeasonEngine,
  applyProgression,
  autoLineup,
  buildLeagueTeams,
...
interface GameState {
  ...
  signFreeAgent: (playerId: number) => boolean
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  signFreeAgent: (playerId) => {
    ...
    if (s.config?.hardCap && totalSalary + fa.salary > capLimit) return false

    const newRoster = [...s.myRoster, fa]
    const tx = makeTransaction('SIGN', `Sign ${fa.name}`, `${fa.pos} · ${fmt$(fa.salary)}/yr`, s.simDate)
    set({
      myRoster: newRoster,
      freeAgentPool: s.freeAgentPool.filter((p) => p.id !== playerId),
      lineup: reconcileLineup(s.lineup, newRoster, [fa.id]),
      transactions: [tx, ...s.transactions].slice(0, 50)
    })
    return true
  },
  ...
  simPlayoffs1Day: () =>
    set((s) => {
      ...
      if (result.bracket.champion) patch.seasonPhase = 'offseason'
      return patch
    }),
  ...
  initDraft: () => set({ draftPhase: 'lottery', draftProspects: generateProspects() }),
