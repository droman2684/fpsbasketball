...
// Runs once, automatically, the moment the season flips to 'offseason' —
// every CPU team with cap room and a real positional need signs the
// best-value fit from the pool, a few rounds each, so the human is competing
// for a shrinking pool exactly like real free agency.
...
// draft picks onto its real roster, runs CPU free agency, enforces roster
// limits, resets standings, regenerates the season calendar, and rolls the
// tradeable draft-pick window forward one year.
export function advanceToNextSeason(
  ...
  const faResult = simulateFreeAgencyPeriod(agedCpuRosters, pool, config, myTeam)
  pool = faResult.freeAgentPool

  const limitResult = enforceRosterLimits(faResult.rosters, waiverPool)

  const offseasonLog: TransactionEntry[] = faResult.signings.slice(0, 8).map((s, i) => ({
    id: `tx-off-${seasonNumber}-${i}`,
    type: 'SIGN',
    headline: `${teamLabel(teams, s.team)} sign ${s.player.name}`,
    detail: `${s.player.pos} · ${fmt$(s.player.salary)}/yr`,
    date: `${seasonEndDate.m}/${seasonEndDate.d}/${seasonEndDate.y}`
  }))

  const records: Record<string, TeamRecord> = {}
  teams.forEach((t) => (records[t.abbr] = { w: 0, l: 0, l10: '0-0' }))

  const nextSeasonStart: SimDate = { y: seasonEndDate.y + 1, m: 10, d: 21 }
